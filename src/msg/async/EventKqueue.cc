...
int KqueueDriver::restore_events() {
  ...
  ldout(cct,21) << __func__ << " on kqfd = " << kqfd << dendl;
  for(i=0;i<size;i++) {
    int num = 0;
    if (sav_events[i].mask == 0 )
      continue;
    ldout(cct,21) << __func__ << " restore kqfd = " << kqfd 
...
int KqueueDriver::test_thread_change(const char* funcname) {
  ...
  if (!pthread_equal(mythread, pthread_self())) {
    ldout(cct,21) << funcname << " We changed thread from " << mythread
  ...
  if (kqfd == -1) {
    kqfd = kqueue();
    ldout(cct,21) << funcname << " kqueue: new kqfd = " << kqfd
                  << " (was: " << oldkqfd << ")"
                  << dendl;
    if (kqfd < 0) {
...
int KqueueDriver::add_event(int fd, int cur_mask, int add_mask)
{
  ...
  ldout(cct,25) << __func__ << " add event kqfd = " << kqfd << " fd = " << fd 
...
int KqueueDriver::del_event(int fd, int cur_mask, int del_mask)
{
  ...
  ldout(cct,25) << __func__ << " delete event kqfd = " << kqfd 
...
{
  ldout(cct,21) << __func__ << " kqfd = " << kqfd << "newsize = " << newsize 
                << dendl;
  if(newsize > sav_max) {
...
int KqueueDriver::event_wait(vector<FiredFileEvent> &fired_events, struct timeval *tvp)
{
  ...
  ldout(cct,21) << __func__ << " kqfd = " << kqfd << dendl;
  ...
  if (tvp != NULL) {
      ...
      ldout(cct,25) << __func__ << " event_wait: " << " NULL" << dendl;
  ...
  if (retval < 0) {
    ...
    ldout(cct,22) << __func__ << " Hit timeout("
                 << timeout.tv_sec << " sec "
                 << timeout.tv_nsec << " nsec"
		 << ")." << dendl;
  } else {
