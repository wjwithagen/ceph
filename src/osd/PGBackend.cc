  ...
  ostream &errorstream)
{
  ...
  for (set<hobject_t, hobject_t::BitwiseComparator>::const_iterator k = master_set.begin();
    ...
    Scrub::ObjectError object_error(*k);

    list<pg_shard_t> auth_list;
    if (auth == maps.end()) {
      object_error.set_auth_missing(maps);
    ...
    for (j = maps.begin(); j != maps.end(); ++j) {
      if (j == auth)
	continue;
      Scrub::ShardError shard_error{j->first};
      if (j->second->objects.count(*k)) {
	shard_error.object = &j->second->objects[*k];
	// Compare
	stringstream ss;
	...
	errorstream << pgid << " shard " << j->first << " missing " << *k
		    << "\n";
	shard_error.set_shard_missing();
      }
      object_error.set_shard_error(shard_error);
    }
    ...
    if (!cur_inconsistent.empty() || !cur_missing.empty()) {
      ...
      Scrub::ShardError error{auth->first, &auth_object};
      object_error.set_shard_error(error);
    }
	...
	error.set_data_digest_mismatch();
	++deep_errors;
	...
	error.set_omap_digest_mismatch();
	++deep_errors;
    ...
    if (object_error.errors)
      store->add_object_error(object_error);
  }
}
