...
TEST_F(TmapMigratePP, DataScan) {
  ...
  ASSERT_EQ(0, ioctx.tmap_put("foo", tmap_trans));

  ASSERT_EQ(0, ioctx.write_full("simple", val));
  ...
  ASSERT_EQ(0, ioctx.omap_set("bar", omap_kvs));
  ...
  ASSERT_EQ(0, ioctx.omap_get_vals("foo", "", 1, &read_vals));
  ...
  read_vals.clear();
  ASSERT_EQ(0, ioctx.omap_get_vals("bar", "", 1, &read_vals));
  ASSERT_EQ(read_vals.size(), 1);
