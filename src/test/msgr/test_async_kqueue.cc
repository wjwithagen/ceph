...
class EventDriverTest : public ::testing::TestWithParam<const char*> {
 public:
  ...
  virtual void SetUp() {
    ...
    cerr << __func__ << " Loading kqueue driver" << std::endl;
    driver = new KqueueDriver(g_ceph_context);
    if (driver != NULL) 
    	driver->init(100);
    else
	cerr << __func__ << " Loading kqueue driver failed" << std::endl;
  }
...
TEST_P(EventDriverTest, PipeTest) {
  ...
  cerr << __func__ << " start pipe" << std::endl;
  r = pipe(fds);
  ASSERT_EQ(r, 0);
  cerr << __func__ << ":" << __LINE__ << " add event" << std::endl;
  r = driver->add_event(fds[0], EVENT_NONE, EVENT_READABLE);
  ASSERT_EQ(r, 0);
  cerr << __func__ << ":" << __LINE__ << " wait event" << std::endl;
  r = driver->event_wait(fired_events, &tv);
  ASSERT_EQ(r, 0);

  cerr << __func__ << " write char." << std::endl;
  char c;
  ...
  cerr << __func__ << " write char again." << std::endl;
  fired_events.clear();
  r = write(fds[1], &c, sizeof(c));
  ASSERT_EQ(r, 1);
  cerr << __func__ << ":" << __LINE__ << " wait event" << std::endl;
  ...
  cerr << __func__ << " events clear." << std::endl;
  fired_events.clear();
  cerr << __func__ << ":" << __LINE__ << " del event" << std::endl;
  driver->del_event(fds[0], EVENT_READABLE, EVENT_READABLE);
  r = write(fds[1], &c, sizeof(c));
  ASSERT_EQ(r, 1);

  cerr << __func__ << ":" << __LINE__ << " event_wait should timeout" << std::endl;
  r = driver->event_wait(fired_events, &tv);
  ASSERT_EQ(r, 0);
}
